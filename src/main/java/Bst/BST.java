...
public class BST {
    ...
    public static boolean isBstHelper(Node<Integer> node, Integer lowerLim, Integer upperLim) {
        ...
        if (node.getLeftChild() != null) {
            isLeftBST = isBstHelper(node.getLeftChild(), lowerLim, node.getData());
        }
        if (isLeftBST && node.getRightChild() != null) {
            isRightBST = isBstHelper(node.getRightChild(), node.getData(), upperLim);
        }
    ...
    public static Node<Integer> createTree(HashMap<Integer, int[]> mapping, int headValue) {
        ...
        for(Integer key : mapping.keySet()) {
            int[] value = mapping.get(key);
            nodes.get(key).setLeftChild(nodes.get(value[0]));
            nodes.get(key).setRightChild(nodes.get(value[1]));
        }
        return head;
    }
}
